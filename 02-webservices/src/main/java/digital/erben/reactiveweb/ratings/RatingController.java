...
public class RatingController {
	...
	public RatingController(RatingRepository repository, UserRetriever userRetriever,
	...
	public Mono<ResponseEntity<Rating>> createRating(@RequestBody Mono<Rating> rating) {
		return rating.doOnSuccess(this::checkIfRatingInBounds).checkpoint("validate").flatMap(this::checkIfUserExists)
				.checkpoint("check-user").flatMap(this::checkIfRestaurantExists).checkpoint("check-restaurant")
				.flatMap(this.repository::save).checkpoint("save").map(ResponseEntity::ok).log("handle-error")
	...
	private Mono<Rating> checkIfRestaurantExists(Rating in) {
		return this.restaurantRetriever.findRestaurant(in.restaurantId())
				.switchIfEmpty(Mono.error(new IllegalArgumentException("Restaurant not found"))).thenReturn(in);
	}

	private Mono<Rating> checkIfUserExists(Rating in) {
		return this.userRetriever.findUser(in.userId())
				.switchIfEmpty(Mono.error(new IllegalArgumentException("User not found"))).thenReturn(in);
	}
