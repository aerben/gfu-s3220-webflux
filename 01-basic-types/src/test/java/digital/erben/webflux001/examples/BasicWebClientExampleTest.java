...
public class BasicWebClientExampleTest {
    ...
    public void callApiViaWebClientToMono() {
        ...
        ExchangeFilterFunction loggingFilterFunction = (request, next) -> {
            System.out.println(request.attribute("myattr").orElse("none"));
            return next.exchange(request);
        };

        Mono<List<City>> cities = WebClient.builder()
            .filter(loggingFilterFunction).build()
            .get()
            .uri(URI.create(URL))
            .attribute("myattr", "myvalue")
