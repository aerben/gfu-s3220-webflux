...
public class IntermediatesExampleTest {
	...
	public void example() {
		...
		var publisher = Flux.<String>generate(sink -> {
			...
			sink.complete();
		}).repeat(10).flatMapIterable(line -> Arrays.asList(line.split(" "))).publish().autoConnect(3);

		var totalCount = publisher.count();

		var wordCountsByFirstCharacter = publisher.groupBy(word -> word.toLowerCase().charAt(0)).flatMap(groupToCount())
				.collectMap(Map.Entry::getKey, Map.Entry::getValue);

		var wordCountsByLength = publisher.groupBy(String::length).flatMap(groupToCount()).collectMap(Map.Entry::getKey,
				Map.Entry::getValue);

		Mono.zip(totalCount, wordCountsByLength, wordCountsByFirstCharacter).log().subscribe(results -> {
