...
public class CitiesServiceTest {
	...
	private City createCity(int id, String city, int population) {
		return new City(id, "wikiDataId" + id, "type", city, city + "Name", "country", "countryCode", "region",
				"regionCode", "regionWdId", 0.0, // latitude
				0.0, // longitude
				population);
	}
	...
	void getCitiesWithLimit() {
		var cities = List.of(createCity(1, "City1", 100000), createCity(2, "City2", 200000));
		when(repository.loadCities()).thenReturn(Flux.fromIterable(cities));

		Flux<City> result = service.getCitiesWithLimit(1);

		StepVerifier.create(result).expectNext(cities.get(0)).verifyComplete();
	}
	...
	void getCitiesWithPopulationGreaterThanInclusive() {
		...
		StepVerifier.create(result).expectNextMatches(city -> city.population() >= 150000)
				.expectNextMatches(city -> city.population() >= 150000).verifyComplete();
	}

	@Test
	void getCitiesSortedByPopulationAscending() {
		var cities = List.of(createCity(3, "City3", 300000), createCity(1, "City1", 100000),
				createCity(2, "City2", 200000));
		when(repository.loadCities()).thenReturn(Flux.fromIterable(cities));

		Flux<City> result = service.getCitiesSortedByPopulationAscending();

		StepVerifier.create(result).expectNext(cities.get(1), cities.get(2), cities.get(0)) // Expected order after
																							// sorting
				.verifyComplete();
	}
	...
	void getFirstCityThatMatches() {
		...
		StepVerifier.create(result).expectNext(cities.get(1)) // First city that matches the predicate
				.verifyComplete();
	}
}
